CREATE DEFINER=`root`@`localhost` TRIGGER `dsp_lpj`.`dsp_ba_lpjk_AFTER_INSERT` 
AFTER INSERT ON `dsp_ba_lpjk` 
FOR EACH ROW
    BEGIN
		...
		IF(SELECT COUNT(*) FROM `ref_satker` WHERE kd_satker = NEW.kd_satker) = 0 THEN
			...
			INSERT INTO `ref_history_satker` (
                ...
                created_at )
			SELECT id_ref_satker, NEW.tahun, NEW.bulan, 1, 1, 0, now()
				FROM `ref_satker`
        ...
        /*
            INSERT INTO debug_trigger
			SELECT DISTINCT '', kd_unit, nm_unit
				FROM ref_unit
		...
		*/
        ...
        SET id_ba_lpj			= NEW.id_ba_lpjk,
            ...
            tahun				= NEW.tahun,
            bulan				= NEW.bulan,
            uang_persediaan		= NEW.saldo_akhir_up,
            ls_bendahara		= NEW.saldo_akhir_lsbend,
            pajak				= NEW.saldo_akhir_pajak,
            pengeluaran_lain	= NEW.saldo_akhir_lain,
            saldo				= NEW.saldo_up,
            kuitansi			= NEW.kuitansi_up,
            saldo_awal			= NEW.saldo_awal_bku,
            saldo_akhir			= NEW.saldo_akhir_bku
            ;
