USE `dsp_lpj`;
DROP TRIGGER IF EXISTS `t_lpjk_refrek_insert`;
DELIMITER $$
CREATE TRIGGER `t_lpjk_refrek_insert` 
AFTER INSERT ON `t_lpjk_refrek` FOR EACH ROW
BEGIN 
	...
	DECLARE id_rekap_lpjk int UNSIGNED;
	DECLARE tahun_lpjk varchar(4);
	DECLARE bulan_lpjk varchar(2);
	...
	-- get bulan tahun
	SELECT DISTINCT id_report_rekap_lpjk, tahun, bulan FROM dsp_report_rekap_lpjk WHERE kd_kppn = NEW.kdkppn GROUP BY id_report_rekap_lpjk, tahun, bulan ORDER BY id_report_rekap_lpjk DESC LIMIT 1 INTO id_rekap_lpjk, tahun_lpjk, bulan_lpjk;
	
	
	INSERT INTO `dsp_report_rekening_lpjk`
	SET id_lpjk_refrek		= NEW.id,
